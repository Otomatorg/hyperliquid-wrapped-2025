...
const OUTPUT_FILE = path.join(
  "hyperEVM",
  "hyperbeatiquidHypeDepositLogs.json"
);
...
const USDT_VAULT = "0x441794D6a8F9A3739F5D4E98a728937b33489D29";
...
async function fetchMintsForRange({ fromBlock, toBlock }) {
  let attempt = 0;
  const maxRetries = 3;
  ...
  while (attempt < maxRetries) {
    ...
    try {
      const logs = await client.getLogs({
        ...
        // Filter: from == zero address (mints)
...
async function main() {
  ...
  console.log(
    `Fetching USDT Vault mint Transfer events (from = zero address) from ${FROM_BLOCK} to ${TO_BLOCK}...`
  );
  ...
  const perRangeLogs = await mapWithConcurrency(
    ...
    async (range, i) => {
      ...
      const logs = await fetchMintsForRange(range);
      console.log(
        `[${i + 1}/${ranges.length}] Found ${logs.length} mint events`
      );
      return logs;
    },
    CONCURRENCY
  ...
  console.log(`\nTotal mint Transfer events found: ${allLogs.length}`);
