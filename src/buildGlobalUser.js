...
function findUniqueUsersFiles(dir, fileList = []) {
  ...
  files.forEach(file => {
    ...
    if (stat.isDirectory()) {
      ...
      findUniqueUsersFiles(filePath, fileList);
    } else if (file.match(/^uniqueUsers.*\.json$/)) {
      // Match uniqueUsers.json, uniqueUsers1.json, uniqueUsers2.json, uniqueUserswVLP.json, etc.
...
async function buildGlobalUserList() {
  try {
    // Find all uniqueUsers*.json files in subdirectories
    ...
    for (const file of files) {
      try {
        ...
        console.log(`  ${path.relative(__dirname, file)}: ${addresses.length} addresses (${added} new unique)`);
      } catch (error) {
    ...
    console.log(`\n✅ Global unique users list saved to: ${path.relative(process.cwd(), outputPath)}`);
